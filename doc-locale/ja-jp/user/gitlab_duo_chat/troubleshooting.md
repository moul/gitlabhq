...
GitLab Duo Self-Hostedでチャットを使用中にこのエラーが発生した場合、AIゲートウェイへの接続に問題がありました。

この問題を解決するには、[セルフホスト型デバッグスクリプト](../../administration/gitlab_duo_self_hosted/troubleshooting.md#use-debugging-scripts)を使用して、AIゲートウェイがGitLabインスタンスからアクセス可能で、期待どおりに機能していることを確認します。
...
このエラーは、AIゲートウェイからイベントが返されないか、GitLabがイベントの解析に失敗した場合に発生します。
...
1. これがモデルのデプロイに関する問題であるかを確認するには、[AIゲートウェイのログ](../../administration/gitlab_duo_self_hosted/logging.md)で特定のエラーメッセージを確認します。

1. 接続を検証するには、AIゲートウェイコンテナで`AIGW_CUSTOM_MODELS__DISABLE_STREAMING`の環境変数を設定してストリーミングを無効にします:
...
このエラーは、AIゲートウェイのプロセスでエラーが発生した場合に発生します。リクエストを再試行してください。
...
## チャット応答のリンクは選択できません {#links-in-chat-responses-are-not-selectable}

GitLab Duo Chatは、外部ウェブサイトおよびサードパーティドメインのURLを、応答で選択可能なリンクとして表示しません。

代わりに、チャットはこれらのタイプのURLを、リンクテキストのみを表示するコード形式のテキストに変換します。宛先URLは表示されません。

この制限により、AI応答で生成される可能性のある悪意のあるリンクからユーザーを保護します。

チャットは、応答で選択可能な次の種類のリンクを表示します:

- `docs.gitlab.com`にあるGitLabドキュメントへのリンク。
...
GitLabクレジットを使い果たすと、Agentic Chatへのアクセスを失う場合があります。

この問題を解決するには、次のいずれかを実行します:

- [GitLabクレジットをさらに購入する](../../subscriptions/gitlab_credits.md#buy-gitlab-credits)。
- 従来のチャットに切り替える。切り替えると、新しい会話が開始されます。以前のAgentic Chatの会話は引き続き表示できますが、読み取り専用です。

### 応答時間が遅い {#slow-response-times}

Agentic Chatは、リクエストの処理とリクエストへの応答に関して、従来のチャットよりも時間がかかる場合があります。

この問題は、Agentic Chatが情報を収集するために複数のAPIコールを行うために発生します。そのため、応答時間が大幅に長くなることがあります。

### 権限の制限 {#limited-permissions}

Agentic Chatは、GitLabユーザーがアクセス権限を持っているリソースと同じリソースにアクセスできます。Agentic Chatがリクエストに回答するために必要なリソースにアクセスできない場合は、[ユーザー権限](../permissions.md)を確認してください。
