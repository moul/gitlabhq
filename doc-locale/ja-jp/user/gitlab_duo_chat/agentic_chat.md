...
- 機能フラグ[`self_hosted_agent_platform`](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/218589) 、[`agentic_chat_ga`](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/219679) 、および[`ai_duo_agent_platform_ga_rollout_self_managed`](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/219679)はGitLab 18.10で削除されました。{{< /history >}}
...
1. チャットテキストボックスに質問を入力し、<kbd>Enter</kbd>キーを押すか、**送信**を選択します。
   - チャットに[コンテキスト](../duo_agent_platform/context.md#gitlab-duo-chat-agentic)を追加できます。
...
- [GitLab for VS Code拡張機能](../../editor_extensions/visual_studio_code/setup.md)をバージョン6.15.1以降でインストールして設定します。
- [GitLab Duo Agent Platformの前提条件](../duo_agent_platform/_index.md#prerequisites)を満たす必要があります。
- [デフォルトのGitLab Duoネームスペース](../profile/preferences.md#set-a-default-gitlab-duo-namespace)を設定します。
...
- [GitLab for Visual Studio拡張機能](../../editor_extensions/visual_studio/setup.md)をバージョン0.60.0以降でインストールして設定します。
- [GitLab Duo Agent Platformの前提条件](../duo_agent_platform/_index.md#prerequisites)を満たす必要があります。
- [デフォルトのGitLab Duoネームスペース](../profile/preferences.md#set-a-default-gitlab-duo-namespace)を設定します。
...
- GitLab UIのチャット履歴の検索機能はGitLab 18.9で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/work_items/582513)。
...
1. チャット履歴で特定の会話を検索するには:
   - GitLab UI: **Search for a thread**テキストボックスに、検索語を入力します。
   - IDE: **Search chats**テキストボックスに、検索語を入力します。
...
## GitLab Duo Chatをローカル環境でカスタマイズする {#customize-gitlab-duo-chat-in-your-local-environment}

GitLab Duo Chatのローカル環境での動作を、コーディングスタイル、チームのプラクティス、プロジェクト要件を反映する指示を提供することでカスタマイズします。
...
- お使いのインスタンスは、GitLab AIゲートウェイに接続されている必要があります。
...
- VS Codeの場合、[GitLab for VS Code拡張機能](../../editor_extensions/visual_studio_code/setup.md)をバージョン6.49.12以降でインストールして設定します。
...
GitLab Duo Chat（エージェント）のレイテンシーを改善するために、プロンプトのキャッシュがデフォルトで有効になっています。プロンプトのキャッシュが有効になっている場合、チャットプロンプトデータは、モデルベンダー（AnthropicまたはVertexAI）によって一時的にメモリに保存されます。プロンプトのキャッシュは、キャッシュされたプロンプトとインプットデータの再処理を回避することで、レイテンシーを大幅に改善します。

### プロンプトキャッシュをオフにする {#turn-off-prompt-caching}

GitLab Duo設定で、トップレベルグループの[プロンプトのキャッシュをオフにする](../duo_agent_platform/code_suggestions/_index.md#turn-off-prompt-caching)ことができます。これにより、コード提案のプロンプトキャッシュもオフになります。
