...
- エピックへのマイルストーンの割り当てはGitLab 18.2で[導入されました](https://gitlab.com/groups/gitlab-org/-/epics/329)。
...
1. 新しいエピックフォームに移動します。
   ...
   - グループ内のエピックから、右上隅にある**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択します。次に、**New related epic**を選択します。
...
1. フィールドに入力します。
   - 説明を入力します。
   - エピックを[非公開にする](#make-an-epic-confidential)には、**非公開に設定**の横にあるチェックボックスを選択します。
   - ラベルを選択します。
...
- プロジェクトのプランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールを持っているか、エピックの作成者であるか、エピックに割り当てられている必要があります。
...
- GitLab 18.5で、[親属性のバルク編集サポートが追加されました。](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/204037)
...
- グループに対して、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールが必要です。

エピックの担当者を変更するには: 

1. トップバーで、**検索または移動先**を選択し、グループを見つけます。
1. **Plan** > **エピック**を選択し、表示したいエピックを選択します。
...
![v17.0で色によって区別されたエピック](img/epic_color_roadmap_v17_0.png)

エピックボードでは、色はエピックのカードアクセントに表示されます: 

![v17.0で関連するエピックの色で強調されたカード](img/epic_accent_boards_v17_0.png)
...
1. 右上隅にある**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択し、次に**Delete epic**を選択します。
...
- 右上隅にある**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択し、次に**Close epic**を選択します。

また、[`/close`クイックアクション](../../project/quick_actions.md#close)を使用することもできます。
...
- 右上隅にある**追加のアクション** ({{< icon name="ellipsis_v" >}}) を選択し、次に**Reopen epic**を選択します。
...
エピックリストに表示される属性を設定するには、[表示設定を構成](../../work_items/_index.md#configure-list-display-preferences)します。
...
- プライベートグループと機密ではないエピック: 以下いずれかのメンバーである必要があります。 - グループ - グループ内のプロジェクト - グループのサブグループのいずれかのプロジェクト
...
- ドロワーと全ページ表示の間の切替はGitLab 18.2で[導入されました](https://gitlab.com/gitlab-org/gitlab/-/issues/536620)。
- GitLab 18.6で[一般提供](https://gitlab.com/gitlab-org/gitlab/-/issues/500367)になりました。機能フラグ`epics_list_drawer`は削除されました。

{{< /history >}}

エピックページまたはエピックボードからエピックを選択すると、詳細パネルが開きます。これにより、エピックリストまたはボードのコンテキストを失うことなく、詳細を表示および編集できます。
...
- エピックを選択し、ドロワーから以下を実行します:
  - 左上隅で、イシューの参照（例：`my_project#123`）を選択します。
  - 右上隅で、**全ページ表示** ({{< icon name="maximize" >}}) を選択します。

イシューを常に全ページ表示で開くには、[アイテムをドロワーで開くかどうかの設定](../../work_items/_index.md#configure-list-display-preferences)を参照してください。
...
> [!note]非公開のエピックには、[非公開のイシュー](../../project/issues/confidential_issues.md)と非公開の子エピックのみを含めることができます。ただし、マージリクエストは、公開プロジェクトで作成された場合は公開されます。非公開のマージリクエストの作成方法については、[非公開のイシューのマージリクエスト](../../project/merge_requests/confidential.md)を参照してください。
...
- **In an existing epic**の場合: 右上隅で、**その他のアクション** ({{< icon name="ellipsis_v" >}}) を選択します。次に、**非公開に設定**を選択します。

また、[`/confidential`クイックアクション](../../project/quick_actions.md#confidential)を使用することもできます。
...
### 子アイテムの表示設定を構成します {#configure-child-item-display-preferences}

{{< history >}}

- 子アイテムの表示オプションはGitLab 18.10で[拡張されました](https://gitlab.com/gitlab-org/gitlab/-/work_items/551231)。

{{< /history >}}

**子アイテム**セクションに表示される情報を構成して、ワークフローにとって最も重要なことに集中できます。

> [!note] 1つのエピックで表示される情報を変更すると、グループとプロジェクトのすべての作業アイテムに対してそれが変更されます。

1. **子アイテム**セクションヘッダーの右上隅にある**表示オプション** ({{< icon name="preferences" >}}) を選択します。

   デフォルトでは、すべてのオプションとフィールドが表示されます。
1. 表示される情報を変更するには、以下の切替をオンまたはオフにします:
...
参加者は、エピックと対話したユーザーです。参加者の表示については、[参加者](../../participants.md)を参照してください。
...
- イシューのプロジェクトとエピックのグループに対して、ゲスト、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールが必要です。
...
- 親エピックのグループまたは子エピックのグループのいずれかに対して、ゲスト、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールが必要です。
...
- 子および親エピックの両方のグループに対して、ゲスト、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールが必要です。
...
- 親エピックのグループに対して、ゲスト、プランナー、レポーター、デベロッパー、メンテナー、またはオーナーロールが必要です。
...
1. エピックで、**子アイテム**セクションで、**追加** > **Add a new epic**を選択します。
...
1. エピックで、**子アイテム**セクションで、**追加** > **Existing epic**を選択します。
