...
> [!note]リポジトリのパスを変更すると、古いURLにプッシュまたはプルしている場合に、ユーザーが問題に遭遇する可能性があります。リダイレクト期間とその副作用の詳細については、[リポジトリの名前変更時のリダイレクト](repository/_index.md#repository-path-changes)を参照してください。
...
> [!warning]プロジェクト削除をスケジュールしたユーザーが、ジョブが実行される前にオーナーロールまたは管理者アクセスを再取得した場合、そのジョブはプロジェクトを完全に削除します。
...
> [!note]デプロイされたPagesは自動的に復元されません。Pagesを復元するには、パイプラインを再実行する必要があります。
...
> [!note]プロジェクトがアーカイブされているか、削除保留中の場合、プロジェクトを転送することはできません。
...
> [!warning]転送プロセス中のエラーは、プロジェクトコンポーネントまたはエンドユーザーの依存関係のデータ損失を引き起こす可能性があります。
