...
> [!note]ダウンストリームプルミラーリポジトリから削除されたアイテムでも、アップストリームリポジトリに残っているものは、次回のプル時に復元されます。例: ミラーリングされたリポジトリのみで削除されたブランチは、次回のプル後に再び表示されます。
...
1. **GitリポジトリURL**を入力します。
   ...
   > [!note] `gitlab`リポジトリをミラーするには、`gitlab.com:gitlab-org/gitlab.git`または`https://gitlab.com/gitlab-org/gitlab.git`を使用します。
...
> [!warning]ブランチをミラーしている場合、このオプションを有効にするとローカルの変更が失われます。
...
- パイプラインはプルミラーリングを設定したユーザーの認証情報を使用するため、セキュリティリスクを考慮してください。たとえば、悪意のあるメンテナーは、次のことができます:
  ...
  - [**リポジトリへのGitプッシュリクエストを許可する**](../../../../ci/jobs/ci_job_token.md#allow-git-push-requests-to-your-project-repository)設定が有効になっている場合、コミットをミラーされたプロジェクトにプッシュすることができます。

> [!warning]この機能は、自身のプロジェクト、または信頼できるメンテナーがいるプロジェクトでのみ有効にしてください。
...
詳細については、[start the pull mirroring process for a project](../../../../api/project_pull_mirroring.md#start-the-pull-mirroring-process-for-a-project)を参照してください。
