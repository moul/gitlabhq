...
1. `ssh://`のURLを入力した場合は、次のいずれかを選択します。
   ...
   - SSH認証でリポジトリミラーを作成するには、[create a mirror with SSH authentication](#example-create-mirror-with-ssh-authentication)の例を参照してください。
...
1. オプション。ミラーリングするブランチの数を制限するには、**保護されたブランチのみミラー**を選択するか、**特定のブランチをミラー**に正規表現を入力します。
1. **ミラーリポジトリ**を選択します。
...
このオプションを使用するには、リポジトリミラーを作成するときに**保護されたブランチのみミラー**を選択します。
...
[RE2正規表現](https://github.com/google/re2/wiki/Syntax)に一致する名前のブランチのみをミラーするには、**特定のブランチをミラー**フィールドに正規表現を入力してください。正規表現に一致しない名前のブランチはミラーリングされません。
...
[プロジェクトアクセストークン](../../settings/project_access_tokens.md)または[グループアクセストークン](../../../group/settings/group_access_tokens.md)の場合、ユーザー名として空白ではない値を使用し、パスワードとしてトークンを使用します。
