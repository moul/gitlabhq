...
- **作業アイテム**機能を無効にすると、プロジェクトユーザーは以下を使用できません:
  - **イシューボード**
  - **サービスデスク**
  - プロジェクトユーザーは、引き続きマージリクエストから**マイルストーン**にアクセスできます。
- **作業アイテム**と**マージリクエスト**を無効にすると、プロジェクトユーザーは以下を使用できません:
  - **ラベル**
  - **マイルストーン**
...
> [!note]プロジェクトアナリティクスを無効にしても、**分析**ナビゲーション項目が削除されるだけで、データは引き続き計算され、対応するAPIエンドポイントを通じて利用できます。
...
1. **設定** > **一般**を選択します。
1. **可視性、プロジェクトの機能、権限**を展開します。
1. **作業アイテム**で、**CVE ID requests in the issue sidebar**切替をオフにしてください。
1. **変更を保存**を選択します。
...
- 次の設定をオンにします。
  ...
  - [マージリクエストのタイトル検証](../merge_requests/title_validation.md)。
  - [自動リベース（マージ前）](../merge_requests/methods/_index.md#automatic-rebase-before-merge)（半線形および早送りマージメソッドの場合）。
  - デフォルトで[**マージリクエスト**が承認されたときにソースブランチを削除](#delete-the-source-branch-on-merge-by-default)オプション
- 次の項目を設定します。
...
- GitLab 17.9で、`extended_expiry_webhook_execution_setting`[フラグ](../../../administration/feature_flags/_index.md)とともに、60日と30日のトリガーがプロジェクトとグループアクセストークンWebhookに[導入](https://gitlab.com/gitlab-org/gitlab/-/issues/499732)されました。デフォルトでは無効になっています。
...
GitLabは、プロジェクトトークンの有効期限が切れる前に、複数の[有効期限メール](project_access_tokens.md#project-access-token-expiry-emails)を送信し、関連する[Webhook](../integrations/webhook_events.md#project-and-group-access-token-events)をトリガーします。デフォルトでは、これらのWebhookはトークンの有効期限が切れる7日前にトリガーされます。
...
1. **設定** > **一般**を選択します。
1. **表示レベル、プロジェクトの機能、権限**セクションを展開します。
1. **Add additional webhook triggers for project access token expiration**チェックボックスを選択します。
1. **変更を保存**を選択します。
