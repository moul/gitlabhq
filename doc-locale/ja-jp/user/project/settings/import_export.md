...
> [!note]プロジェクトエクスポートファイルをデータのバックアップに使用しないでください。バックアップにプロジェクトのエクスポートファイルを使用しても、必ずしも機能するとは限らず、すべての項目がエクスポートされるわけではありません。
...
- プッシュルール
- 絵文字リアクション
- 直接プロジェクトメンバー（エクスポートされたプロジェクトのグループに対するメンテナーまたはオーナーロールがある場合）
...
- 安全なファイル
- [Git関連イベントのアクティビティログ](https://gitlab.com/gitlab-org/gitlab/-/issues/214700)（たとえば、プッシュやタグの作成）
...
> [!warning]信頼できるソースからのプロジェクトのみをインポートしてください。信頼できないソースからプロジェクトをインポートすると、攻撃者が機密データを盗む可能性があります。
...
> [!warning]この機能はGitLab 14.6で[非推奨](https://gitlab.com/groups/gitlab-org/-/epics/4619)になり、[直接転送によるグループの移行](../../group/import/_index.md)に置き換えられました。ただし、この機能は[オフライン環境](../../application_security/offline_deployments/_index.md)での移行には依然として推奨されます。オフラインインスタンス間の移行のサポートは、[エピック8985](https://gitlab.com/groups/gitlab-org/-/epics/8985)で提案されています。
