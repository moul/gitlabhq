...
> [!note] GitLab.comでは、カスタムメールボックスが`contact-project+%{key}@incoming.gitlab.com`をメールアドレスとして既に設定されています。プロジェクト設定で[カスタムサフィックス](#configure-a-suffix-for-service-desk-alias-email)を設定することは引き続き可能です。
