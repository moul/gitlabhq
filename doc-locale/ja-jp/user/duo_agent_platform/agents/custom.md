...
> [!flag]メンテナーとしてプロジェクトでエージェントを直接有効化する機能は、機能フラグによって制御されます。詳細については、履歴を参照してください。
...
1. オプション。**利用可能なツール**の下にある**ツール**ドロップダウンリストから、エージェントがアクセスできるツールを選択します。たとえば、エージェントにイシューを自動的に作成させるには、**イシューを作成**を選択します。
   ...
   > [!note]一部のツールはIDE拡張機能を必要とし、Web UIでは利用できません。詳細については、[エージェントツール](tools.md)のリストを参照してください。
1. **エージェントを作成**を選択します。
