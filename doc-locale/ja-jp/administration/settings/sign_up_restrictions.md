...
制限付きアクセスをオンにすると、[グループ階層外のグループを招待できないようにする](../../user/project/members/sharing_projects_groups.md#prevent-inviting-groups-outside-the-group-hierarchy)設定が自動的にオンになります。この設定により、予期せずに新しい請求対象ユーザーが追加されて超過料金が発生してしまう事態を防ぐことができます。
...
### SAML、SCIM、およびLDAPによるプロビジョニングの動作 {#provisioning-behavior-with-saml-scim-and-ldap}

{{< history >}}

- GitLab 18.6で`bso_minimal_access_fallback`[フラグ](../feature_flags/_index.md)とともに[導入](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/206932)されました。デフォルトでは無効になっています。
...
最小アクセスロールを持つユーザーは、認証してグループにアクセスできますが、[制限された権限](../../user/permissions.md#users-with-minimal-access)があります。コード行が利用可能になると、ユーザーは意図したアクセスレベルにプロモートできるようになります。請求対象ロールを持つ既存のユーザーは、この動作の影響を受けません。

[コード行の使用状況を表示](../../subscriptions/manage_users_and_seats.md#view-seat-usage)し、最小アクセスを持つユーザーを管理できます。
...
ユーザー上限とは、管理者の承認なしにサブスクリプションへ登録または追加できる請求対象ユーザーの最大数を指します。ユーザー上限に達した場合、新たに登録または追加されたユーザーは、管理者の[承認](../moderate_users.md#approve-or-reject-a-user-sign-up)を受ける必要があります。ユーザーは、管理者による承認後にのみアカウントを利用できます。
...
## ユーザー上限から制限付きアクセスへの変更 {#changing-from-user-cap-to-restricted-access}
...
デフォルトでは、ユーザーパスワードに設定されている[要件](../../user/profile/user_passwords.md#password-requirements)は限定的です。最小文字数を増やしたり、特定の種類の文字を必須にしたりするなど、要件を変更できます。
...
1. 複雑さの要件を変更します:
   ...
   | 設定 | 説明 |
   |---------|-------------|
   | **パスワードの最小文字数** | 必要な最小文字数を設定します。8文字以上128文字以下にする必要があります。 |
...
- 機能フラグ`member_promotion_management`は、GitLab 18.0で[削除](https://gitlab.com/gitlab-org/gitlab/-/merge_requests/187888)されました。
