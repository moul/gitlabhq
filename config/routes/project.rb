...
constraints(Projects::ProjectUrlConstraint.new) do
  ...
  scope(
    ...
    scope(
      ...
      # Legacy repository archive URL (/{namespace}/{project}/repository/archive.{format}).
      # Generated by npm's hosted-git-info for `gitlab:` shorthand dependencies.
      # Returns 404 to prevent the catch-all route from redirecting to sign-in page,
      # which breaks automated tools like npm that expect a proper HTTP error or tarball.
      # See https://gitlab.com/gitlab-org/gitlab/-/issues/595699
      # rubocop:disable Cop/PutProjectRoutesUnderScope -- Legacy route must live outside /-/ scope to match npm's hosted-git-info URL format
      get 'repository/archive',
        format: true,
        constraints: { format: Gitlab::PathRegex.archive_formats_regex },
        to: 'legacy_archive#show'
      # rubocop:enable Cop/PutProjectRoutesUnderScope
